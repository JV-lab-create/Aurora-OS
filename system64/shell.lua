...
if #tArgs > 0 then
    ...
    print("Aurora OS Shell 1.0")
    term.setTextColour(colors.lightGray)
    print("PC Firmware Version: " .. os.version() )
    ...
    while not bExit do
        ...
        if not(os.getComputerLabel() == nil) then
            write(os.getComputerLabel() .. "@auroraos") -- TO DO put computer label or computer id at the end of the write line
        else
            write("PC#" .. os.computerID() .. "@auroraos")
        end
        ...
        write("~/")
        term.setTextColour( colors.blue )
        if not(shell.dir() == "" or shell.dir() == "/") then
            write(shell.dir() .. "/") 
        else
            write("/")
        end
